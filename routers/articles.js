...
router.get('/:query', (req, res, next) => {
    ...
    page_type = req.originalUrl.split("/")[1].split("?")[0];
        mysql.$query('SELECT p.created_at, p.page_id, p.page_type, pc.page_content_id, pc.last_update, pc.title, pc.content, c.nick creator, c.user_id creator_id, e.nick editor, e.user_id editor_id, i.image_id image_id, i.file_format file_format FROM pages p LEFT JOIN page_content pc ON p.page_content_id = pc.page_content_id OR p.page_id = pc.page_id AND p.page_content_id IS NULL LEFT JOIN users c ON p.user_id = c.user_id LEFT JOIN users e ON pc.user_id = e.user_id LEFT JOIN images i ON i.image_id = pc.image_id OR i.type = "banner" AND i.type_id = p.page_id WHERE title = ? AND p.page_type = ? AND p.hidden = 0 ORDER BY i.image_id DESC;', [req.params.query, page_type], {
        req, res, next, async handler(error, result, fields, router) {
            if(error) return next(new errorHandling.SutekinaError(error.message, 500));
